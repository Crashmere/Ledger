...
- 概览、账户、报告、搜索由 `App.vue` 统一展示居中月份条，状态使用 `useSelectedMonth` 跨页共享，仅会话内保留；上下界为全账本最早存活交易月（含专项）至当前月。
- 账户明细、分类支出与流入流出按共享月份查询，余额保持全期；报告采用最后操作的月份或原时间范围，不取交集、不反向修改共享月份；搜索默认全部时间，勾选月份过滤才限月。
- 其他页面顶栏控制通过 `#topbar-slot` Teleport 右对齐；账户/标签 tab 在手机端移到内容顶部，不能与月份条重叠。
