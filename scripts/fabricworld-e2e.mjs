...
const fabricRoot = resolve(
  process.env.FABRICWORLD_CHECKOUT || resolve(root, "../FabricWorld"),
);
const { chromium } = createRequire(resolve(fabricRoot, "web/package.json"))(
  "playwright",
);
...
const gateway = http.createServer((req, res) => {
  ...
  const upstream = http.request(
    {
      hostname: "127.0.0.1",
      port: fabric ? 19082 : 19081,
      method: req.method,
      path: req.url.replace(fabric ? /^\/fabricworld/ : /^\/ledger/, ""),
      headers: req.headers,
    },
    (response) => {
...
try {
  execFileSync(resolve(root, "bin/ledger"), [
    "init",
    "--db",
    resolve(data, "ledger.sqlite"),
  ]);
  execFileSync(resolve(fabricRoot, "bin/fabricworld"), [
    "init",
    "--data",
    resolve(data, "fabric"),
  ]);
  processes.push(
    spawn(
      ...
      ],
      {
        env: {
          ...process.env,
          LEDGER_FABRICWORLD_URL: "http://127.0.0.1:19082",
        },
        stdio: "ignore",
      },
    ),
  ...
  processes.push(
    spawn(
      resolve(fabricRoot, "bin/fabricworld"),
      [
        "serve",
        "--data",
        resolve(data, "fabric"),
  ...
  await new Promise((r, reject) =>
    gateway.once("error", reject).listen(19080, "127.0.0.1", r),
  );
  await ready(base + "/ledger/healthz");
  await ready(base + "/fabricworld/healthz");
  const account = await api("/ledger/api/accounts", {
    name: "副业",
    color: -1,
    initialBalance: 0,
  ...
  const category = await api("/ledger/api/categories", {
    accountId: account.id,
    name: "纺织",
    color: -1,
  });
  const other = await api("/ledger/api/accounts", {
    name: "日常",
    color: -1,
    initialBalance: 0,
    includeInBalance: true,
  ...
  await api("/ledger/api/categories", {
    accountId: other.id,
    name: "纺织",
    color: -1,
  });
  ...
  for (const width of [320, 375, 1440]) {
    const page = await browser.newPage({
      viewport: { width, height: width === 1440 ? 1000 : 667 },
    });
    const errors = [];
    page.on("pageerror", (e) => errors.push(e.message));
    const count = async () =>
      (await api("/fabricworld/api/fabrics?status=all")).total;
    ...
    for (const [name, id, type] of [
      ...
      const txn = await save(name, id, type);
      assert.equal(txn.fabricWorldEligible, false);
    ...
    await page.screenshot({
      path: resolve(output, "confirm-" + width + ".png"),
    });
    let attempts = 0;
    await page.route("**/api/transactions/*/fabricworld", async (route) => {
      ...
      if (attempts === 1) {
        await route.fulfill({
          ...
          body: '{"error":{"code":"FABRICWORLD","message":"测试临时失败"}}',
        });
        return;
      }
      if (attempts === 2) {
    ...
    await page.getByRole("button", { name: "是，重试", exact: true }).click();
    await page
      .getByRole("button", { name: "是，前往编辑", exact: true })
      .waitFor();
    assert.equal(attempts, 3);
    assert.equal(await count(), before + 1);
    await page.screenshot({
      path: resolve(output, "success-" + width + ".png"),
    });
    await page
      .getByRole("button", { name: "是，前往编辑", exact: true })
      .click();
    ...
    assert.equal(
      await page.getByLabel("布料名称", { exact: true }).inputValue(),
      "同步测试棉布 " + width,
    );
    assert.equal(
      await page.getByLabel("购买日期", { exact: true }).inputValue(),
      "2026-09-18",
    );
    assert.equal(
      await page.getByLabel("购买总价 / 元", { exact: true }).inputValue(),
      "123.45",
    );
    ...
    await page.screenshot({
      path: resolve(output, "editor-" + width + ".png"),
      fullPage: true,
    });
    if (width === 375) {
      const photo = resolve(data, "synthetic.jpg");
      execFileSync("vips", ["black", photo, "32", "32", "--bands", "3"]);
      await page.locator('input[capture="environment"]').setInputFiles(photo);
      await page.getByLabel("布料名称", { exact: true }).fill("已补充照片");
      const saved = page.waitForResponse(
        (r) =>
          /\/api\/fabrics\/[a-f0-9]{32}$/.test(r.url()) &&
      ...
      const replay = await api(
        "/ledger/api/transactions/" + txn.id + "/fabricworld",
        {},
      );
      const enriched = await api("/fabricworld/api/fabrics/" + replay.fabricId);
      assert.equal(enriched.name, "已补充照片");
    ...
    await page
      ...
      .waitFor();
    await page.getByRole("button", { name: "否，返回记账" }).click();
    assert.ok(page.url().includes("/ledger/transactions/add"));
    assert.equal(await count(), before + 2);
    await page.route("**/api/transactions/*/fabricworld", (route) =>
      route.abort("failed"),
    );
    await save("失败后留在记账");
    await page.getByRole("button", { name: "是，同步", exact: true }).click();
    ...
    if (width === 375) {
      ...
      await page
        .getByLabel("金额 标题 备注", { exact: true })
        .fill("12 批量棉布\n13 批量麻布\n+5 退款");
      await page
        .getByLabel("默认分类", { exact: true })
        .selectOption(category.id);
      await page
        .getByRole("button", { name: "生成待记账清单", exact: true })
        .click();
      await page
        .getByRole("button", { name: "确认记账 3 笔", exact: true })
        .click();
      await page.getByRole("button", { name: "保存", exact: true }).click();
      await page
        .getByRole("dialog", { name: "是否同步到 FabricWorld？" })
        .waitFor();
      assert.ok(
        await page
          .locator("dialog[open]")
          .innerText()
          .then((t) => t.includes("批量棉布")),
      );
      await page.getByRole("button", { name: "否，返回记账" }).click();
      assert.ok(
        await page
    ...
    const existing = await api("/ledger/api/transactions", {
      type: "expense",
      amount: 8765,
      accountId: account.id,
    ...
    page.on("request", (r) => {
      if (
        ...
        /\/api\/transactions(?:\/[a-f0-9-]+)?$/.test(new URL(r.url()).pathname)
      )
        transactionWrites.push(r.url());
    });
    await page.goto(base + "/ledger/txn/" + existing.id + "/edit");
    const syncButton = page.getByRole("button", {
      name: "同步至 FabricWorld",
      exact: true,
    });
    ...
    await page.screenshot({
      path: resolve(output, "existing-detail-" + width + ".png"),
    });
    await syncButton.click();
    await page
      .getByRole("button", { name: "否，返回详情", exact: true })
      .click();
    ...
    await page
      .getByText("请先保存修改，再同步至 FabricWorld。", { exact: true })
      .waitFor();
    ...
    await page
      .locator(".transaction-line")
      .filter({ hasText: existing.title })
      .click();
    await syncButton.waitFor();
    await syncButton.scrollIntoViewIfNeeded();
    assert.ok(
    ...
    await page.screenshot({
      path: resolve(output, "search-detail-" + width + ".png"),
    });
    await syncButton.click();
    await page
      .getByRole("button", { name: "否，返回详情", exact: true })
      .click();
    assert.ok(page.url().includes("/ledger/transactions"));
    await syncButton.click();
    await page.getByRole("button", { name: "是，同步", exact: true }).click();
    await page
      .getByRole("button", { name: "是，前往编辑", exact: true })
    ...
    const nonTarget = await api("/ledger/api/transactions", {
      type: "income",
      amount: 100,
      accountId: account.id,
      categoryId: category.id,
    ...
    await page
      .locator(".transaction-line")
      .filter({ hasText: nonTarget.title })
      .click();
    assert.equal(await syncButton.count(), 0);
    ...
    console.log(
      "PASS " +
        width +
        "px: cancel, non-target, failure, lost response, retry, mapping, edit navigation",
    );
  }
} finally {
  await browser?.close();
